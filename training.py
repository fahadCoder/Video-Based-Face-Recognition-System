...
def main(args):
    ...
    #     # clustering.fit()
    #     clustering.fit(seed=0)
    #     clustering.save()

        
    #     clustering.print_objective_table()

    #     clustering.analyze_initialization_sensitivity(num_runs=5)
    if args.mode == ReIdMode.CLUSTER:
        # Fit multiple initializations, choose best
        best_seed = None
        best_J = float("inf")
        best_centers = None
        ...
        for s in range(10):
            clustering.fit(seed=s)
            J = clustering.objective_history[-1]
            if J < best_J:
                best_J = J
                best_seed = s
                best_centers = clustering.cluster_center.copy()
                best_membership = list(clustering.cluster_membership)

        # restore best solution + save it
        clustering.cluster_center = best_centers
        clustering.cluster_membership = best_membership
        print(f"Saving best clustering (seed={best_seed}, final J={best_J:.6f})")
        clustering.save()
        print("Total embeddings used for clustering:", clustering.embeddings.shape[0])


        # optional: show table for the best run
        clustering.fit(seed=best_seed)
        clustering.print_objective_table()
        clustering.analyze_initialization_sensitivity(num_runs=10)
